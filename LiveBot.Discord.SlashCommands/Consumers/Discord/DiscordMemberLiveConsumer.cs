        ...
        public async Task Consume(ConsumeContext<IDiscordMemberLive> context)
        {
            ...
            foreach (var guild in user.MutualGuilds)
            {
                ...
                catch (Exception ex)
                {
                    _logger.LogError(
                        exception: ex,
                        message: "Unknown error processing DiscordMemberLive event for {Username} ({UserId}) {GuildName} ({GuildId})",
                        Format.UsernameAndDiscriminator(user: user, doBidirectional: true),
                        guild.Id.ToString(),
                        user.Id.ToString(),
                        guild.Name
                    );
